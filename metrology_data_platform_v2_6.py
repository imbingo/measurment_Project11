...
def page_dashboard(user, query=None):
    ...
    image_config_issue_count = len(image_config_issues)
    config_issue_html = "".join(
        f"<tr><td>{h(r['issue'])}</td><td>{h(r['production_code'])}</td><td>{h(r['item_name'])}</td><td><a class='btn secondary' href='/item_edit?id={r['id']}'>编辑量测项</a></td></tr>"
        for r in config_issue_rows[:10]
    ) or "<tr><td colspan='4'>暂无配置完整性问题</td></tr>"
    ...
    <div class="card"><h2>配置检查</h2><p class="note">无工序配置：{no_process_count} ｜ 无启用指标：{no_metric_count} ｜ 数据源路径为空：{empty_path_count} ｜ Image OCR 配置问题：{image_config_issue_count}</p><div class="table-wrap"><table><tr><th>问题</th><th>生产编号</th><th>量测项</th><th>处理</th></tr>{config_issue_html}</table></div></div>
...
def page_results(user, query=None):
    ...
    for r in rows:
        ...
        row_parts.append(f"""
        <tr>
          <td><input form="bulkVoidResultsForm" type="checkbox" name="result_ids" value="{r['id']}"></td>
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/version":
            self.send_html(f"<h1>Metrology Config App {h(APP_VERSION)}</h1><p>PORT={h(PORT)}</p><p>CSV/XLSX/Image OCR collection + shared image folder routing + multi-sheet template wizard + dashboard action cards + voided result traceability</p>")
            return
